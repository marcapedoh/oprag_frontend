...
export const certificatControlReducer = createReducer(
  ...
  on(getStatWithoutInspection, (state) => ({ ...state })),
  on(getStatWithoutInspectionSuccess, (state, { stat }) => ({ ...state, stat })),
  on(getStatWithoutInspectionFailure, (state, { error }) => ({ ...state, error }))
)
