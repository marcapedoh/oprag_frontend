...
export class CertificatControlEffects {
  ...
  getStatWithoutInspection$ = createEffect(() =>
    this.actions$.pipe(
      ofType(getStatWithoutInspection),
      concatMap(({ dateDebut, dateFin }) =>
        this.dataService.getStatWithoutInspection(dateDebut, dateFin).pipe(
          map((stat) => getStatWithoutInspectionSuccess(stat)),
          catchError((error: string) => of(getStatWithoutInspectionFailure(error)))
        ))
    )
  )
